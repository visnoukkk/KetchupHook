-- KetchupHook UI Library (based on LinoriaLib)
-- Modified to have centered title, game name on right, and center notifications

local cloneref = (typeof(cloneref) == "function" and cloneref)
    or (typeof(clonereference) == "function" and clonereference)
    or function(instance) return instance end
...
do
    local ok, result = pcall(function()
        return cloneref(LocalPlayer:GetMouse())
    end)
    Mouse = ok and result or nil
end

-- declare getgenv BEFORE any use to prevent obfuscator hoisting nil crash
local getgenv
do
    local ok, result = pcall(function()
        return (typeof(getgenv) == "function" and getgenv) or function() return shared end
    end)
    getgenv = ok and result or function() return shared end
end
...
if typeof(getgenv().Drawing) == "table" then
    DrawingLib = getgenv().Drawing
end
local ProtectGui
do
    local ok, fn = pcall(function()
        return protectgui or (syn and syn.protect_gui)
    end)
    ProtectGui = (ok and fn) or function() end
end
local GetHUI
do
    local ok, fn = pcall(function()
    ...
    GetHUI = (ok and fn) or function() return CoreGui end
end
...
local Library = {
    ...
    -- KetchupHook Theme – dark + red accent
    FontColor = Color3.fromRGB(255, 255, 255);
    MainColor = Color3.fromRGB(30, 30, 30);
    BackgroundColor = Color3.fromRGB(20, 20, 20);

    AccentColor = Color3.fromRGB(230, 30, 30);      -- vibrant red
    DisabledAccentColor = Color3.fromRGB(120, 120, 120);

    OutlineColor = Color3.fromRGB(60, 60, 60);
    DisabledOutlineColor = Color3.fromRGB(80, 80, 80);
    ...
    -- notification --
    Notify = nil;
    NotifySide = "Center";  -- default to center
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                ChangedConn = InputService.InputChanged:Connect(function(Change)
                    if Change.UserInputType == Enum.UserInputType.MouseMovement or Change == Input then
                        ...
                        if IsMainWindow and Library.WireframeDrag then
                            -- first movement threshold — spawn the blue outline ghost
                            if not HasMoved and Delta.Magnitude > 2 then
                                HasMoved = true

                                Wireframe = Instance.new("Frame")
                                Wireframe.Size               = Instance.Size
                                Wireframe.Position           = Instance.Position
                                Wireframe.AnchorPoint        = Instance.AnchorPoint
                                Wireframe.BackgroundTransparency = 1
                                Wireframe.Active             = false
                                Wireframe.ZIndex             = 100000
                                Wireframe.Parent             = ScreenGui

                                local Stroke = Instance.new("UIStroke")
                                Stroke.Color            = Library.AccentColor
                                Stroke.Thickness        = 2
                                Stroke.ApplyStrokeMode  = Enum.ApplyStrokeMode.Border
                                Stroke.Parent           = Wireframe
                            end

                            -- move the ghost, not the real window
                            ...
                            -- WireframeDrag off: classic live drag
                ...
                EndedConn = InputService.InputEnded:Connect(function(EndInput)
                    if EndInput == Input or EndInput.UserInputType == Enum.UserInputType.Touch then
                        ...
                        if IsMainWindow and Library.WireframeDrag and HasMoved and Wireframe then
                            -- snap real window to where the ghost landed
...
function Library:AddToolTip(InfoStr, DisabledInfoStr, HoverInstance)
    ...
    if LibraryMainOuterFrame then
        GiveSignal(LibraryMainOuterFrame:GetPropertyChangedSignal("Visible")
...
do
    local BaseAddonsFuncs = {}
        ...
        assert(Info.Default, string.format("AddKeyPicker (IDX: %s)
    ...
    function BaseAddonsFuncs:AddColorPicker(Idx, Info)
        ...
        assert(Info.Default, string.format("AddColorPicker (IDX: %s)
        ...
        do
            ...
            ContextMenu.Inner.Layout:GetPropertyChangedSignal("AbsoluteContentSize")
    ...
    function BaseAddonsFuncs:AddDropdown(Idx, Info)
        ...
        assert(Info.Values, string.format("AddDropdown (IDX: %s)
        ...
        function Dropdown:BuildDropdownList()
            ...
            for Idx, Value in next, Values do
                ...
                if Info.Searchable and not string.lower(StringValue)
                    continue
                end
...
do
    ...
    function BaseGroupboxFuncs:AddSlider(Idx, Info)
        assert(Info.Default,    string.format("AddSlider (IDX: %s)
        assert(Info.Text,       string.format("AddSlider (IDX: %s)
        assert(Info.Min,        string.format("AddSlider (IDX: %s)
        assert(Info.Max,        string.format("AddSlider (IDX: %s)
        assert(Info.Rounding,   string.format("AddSlider (IDX: %s)
...
--// Notifications \\--
do
    -- Left and right areas kept for compatibility, but default to center
    ...
    -- AnchorPoint (0.5, 1) + Position (0.5, 0, 1, -10) pins the BOTTOM EDGE of this
    -- frame to 10px above the screen bottom, horizontally centered.
    -- The UIListLayout stacks children upward via VerticalAlignment.Bottom.
    ...
    function Library:Notify(...)
        ...
        -- If side is not left or right, default to center
        if Side ~= "left" and Side ~= "right" then Side = "center" end
        ...
        local NotifyOuter = Library:Create("Frame", {
            ...
            AnchorPoint = Side == "center" and Vector2.new(0.5, 0) or Vector2.zero;
            ClipsDescendants = true;
            ZIndex = 11000;
            Visible = false;
            Name = "Notif";
            Parent = Side == "left" and Library.LeftNotificationArea 
                    or Side == "right" and Library.RightNotificationArea 
                    or Library.CenterNotificationArea;
        })
        ...
        local ExtraWidth = 0
        local TextPosition = Side == "left" and UDim2.new(0, 4, 0, 0) or Side == "right" and UDim2.new(1, -4, 0, 0) or UDim2.new(0, 4, 0, 0)
        local TextSizeOffsetX = -4
        local TextSizeOffsetY = 0
        local TextXAlignment = Side == "left" and Enum.TextXAlignment.Left or Side == "right" and Enum.TextXAlignment.Right or Enum.TextXAlignment.Left
        ...
        if Data.Icon then
            ...
            if ParsedIcon then
                ...
                IconLabel = Library:Create("ImageLabel", {
                    BackgroundTransparency = 1,
                    AnchorPoint = Vector2.new(0, 0.5),
                    Position = (Side == "left" or Side == "center") and UDim2.new(0, 6, 0.5, 0) or UDim2.new(0, 4, 0.5, 0),
        ...
        local NotifyLabel = Library:CreateLabel({
            AnchorPoint = Side == "left" and Vector2.new(0, 0) or Side == "right" and Vector2.new(1, 0) or Vector2.new(0, 0);
        ...
        local SideColor = Library:Create("Frame", {
            AnchorPoint = (Side == "left" or Side == "center") and Vector2.new(0, 0) or Vector2.new(1, 0);
            Position = (Side == "left" or Side == "center") and UDim2.new(0, -1, 0, -1) or UDim2.new(1, -1, 0, -1);
            BackgroundColor3 = Library.AccentColor;
            BorderSizePixel = 0;
...
function Library:CreateWindow(...)
    ...
    Library.NotifySide = WindowInfo.NotifySide or "Center"
    ...
    -- Title bar with centered title and game name on right
    ...
    if typeof(WindowInfo.GameName) == "string" and WindowInfo.GameName ~= "" then
        ...
        pcall(function()
            GameNameLabel.Text = game:GetService("MarketplaceService")
        end)
    end
    ...
    function Window:AddDialog(Idx, Info)
        ...
        function Dialog:Dismiss()
            TweenService:Create(DialogScale, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Scale = 0.95 })
            TweenService:Create(DialogOverlay, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundTransparency = 1 })
        ...
        function Dialog:AddFooterButton(ButtonIdx, ButtonInfo)
            ...
            local ButtonWrap = {
                ...
                SetDisabled = function(self, Disabled)
                    ...
                    if Disabled then
                        TweenService:Create(TextBtn, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundTransparency = 0.5 })
                        TweenService:Create(BtnLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { TextTransparency = 0.5 })
                    else
                        TweenService:Create(TextBtn, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundTransparency = 0 })
                        TweenService:Create(BtnLabel, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { TextTransparency = 0 })
                    end
                end
            }
...
do
        ...
        for _, Side in next, { LeftSide, RightSide } do
            Side:WaitForChild("UIListLayout"):GetPropertyChangedSignal("AbsoluteContentSize")
    ...
    function Window:Toggle(Toggling)
        ...
        for _, Desc in next, Outer:GetDescendants() do
            ...
            for _, Prop in next, Properties do
                ...
                TweenService:Create(Desc, TweenInfo.new(FadeTime, Enum.EasingStyle.Linear), { [Prop] = Toggled and Cache[Prop] or 1 })
            end
        end
...
local function OnPlayerChange()
    ...
    for _, Value in next, Options do
        if Value.SetValues and Value.Type == "Dropdown" and Value.SpecialType == "Player" then
            local list = Value.ExcludeLocalPlayer
                and (Value.ReturnInstanceInstead and ExcludedPlayerList or StringExcludedPlayerList)
                or  (Value.ReturnInstanceInstead and PlayerList         or StringPlayerList)
...
----
-- Expose the library as KetchupHook (and keep Linoria for compatibility)
getgenv().KetchupHook = Library
if getgenv().skip_getgenv_linoria ~= true then getgenv().Library = Library end
getgenv().Linoria = Library  -- optional, for scripts that expect it
return Libraryend
