...
local Library = {
    ...
    WireframeDrag = true; -- when dragging the main window, show a blue outline and only snap the window into place on release
    WireframeColor = Color3.fromRGB(0, 85, 255);
...
--// Creates (once) a borderless frame with a blue UIStroke that stands in for
--// Target while it's being dragged. Only ever used for the main window.
--// NOTE: the parameter is deliberately not named "Instance" - that name is used
--// elsewhere in this file as an argument name and would shadow the global
--// Instance.new constructor used below.
function Library:CreateDragWireframe(Target)
    local Wireframe = Instance.new("Frame")
    Wireframe.Name = "DragWireframe"
    Wireframe.AnchorPoint = Target.AnchorPoint
    Wireframe.Position = Target.Position
    Wireframe.Size = Target.Size
    Wireframe.BackgroundTransparency = 1
    Wireframe.BorderSizePixel = 0
    Wireframe.Active = false
    Wireframe.ZIndex = 12000
    Wireframe.Parent = ScreenGui

    local Stroke = Instance.new("UIStroke")
    Stroke.Name = "Outline"
    Stroke.Color = Library.WireframeColor
    Stroke.Thickness = 2
    Stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    Stroke.Parent = Wireframe

    Library:AddToRegistry(Stroke, { Color = "WireframeColor" }, true)
    ...
    if Library.IsMobile == false then
        Instance.InputBegan:Connect(function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                local UseWireframe = IsMainWindow == true and Library.WireframeDrag == true
                local Wireframe = nil
                ...
                while InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
                    ...
                    if UseWireframe then
                        if not Wireframe then
                            Wireframe = Library:CreateDragWireframe(Instance)
                        end

                        Wireframe.Position = NewPosition
                    else
        ...
        InputService.TouchMoved:Connect(function(Input)
            ...
            if Input == DraggingInput and Dragging and (IsMainWindow == true and (Library.CanDrag == true and Library.Window.Holder.Visible == true) or true) then
                ...
                if IsMainWindow == true and Library.WireframeDrag == true then
                    if not Wireframe then
                        Wireframe = Library:CreateDragWireframe(Instance)
                    end
...
--// Notifications \\--
do
    -- Single notification tray anchored to the bottom-center of the screen.
    -- New notifications are appended at the bottom edge and push older ones up.
    ...
    -- Kept for backwards compatibility with scripts calling this - notifications
    -- are now always centered at the bottom of the screen, so this is a no-op.
